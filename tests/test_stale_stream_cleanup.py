...
def test_service_worker_cache_bumped_for_frontend_fix_delivery(tmp_path):
    """Changed frontend bytes invalidate the cache even at the same Git version."""
    import re
    from api.asset_versions import render_service_worker

    asset = tmp_path / 'sessions.js'
    asset.write_text('old sessions code')
    before = render_service_worker(SW_SRC, tmp_path, 'fixed-version')
    asset.write_text('new sessions code')
    after = render_service_worker(SW_SRC, tmp_path, 'fixed-version')
    def cache_name(text):
        match = re.search(r"const CACHE_NAME = '([^']+)';", text)
        assert match is not None
        return match.group(1)
    assert cache_name(before).startswith('hermes-shell-sha256-')
    assert cache_name(before) != cache_name(after)
    assert 'keys.filter((k) => k !== CACHE_NAME)' in SW_SRC
