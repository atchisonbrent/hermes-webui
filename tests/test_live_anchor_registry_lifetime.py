...
const S={busy:true,activeStreamId:streamId};
const _isSessionCurrentPane=()=>true;
const _deferredStreamRecoveryBound=false;
...
timers.shift().fn();
assert.equal(_anchorRegistryMap.get(streamId),_anchorRegistry,'paused current run expired');
S.busy=false;
timers.shift().fn();
